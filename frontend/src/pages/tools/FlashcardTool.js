...
export default function FlashcardTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/flashcards', form);
  ...
  return (
    <ToolPage icon="🃏" title="AI Flashcard Generator" desc="Notes paste karo — flashcards, quiz, mind map pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Subject</label>
            <input className="input-field" type="text" name="subject" value={form.subject}
              onChange={handleChange} placeholder="World War 2, Data Structures, Organic Chemistry..." required />
          </div>
          <div className="form-group">
            <label>Lecture Notes Paste Karo</label>
            <textarea className="input-field" name="notes" value={form.notes} onChange={handleChange}
              placeholder="Apne notes yahan paste karo..." rows="4" required />
          </div>
