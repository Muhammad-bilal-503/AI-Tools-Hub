...
export default function InterviewTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/interview', form);
  ...
  return (
    <ToolPage icon="🎤" title="AI Interview Coach" desc="Job role select karo — mock interview practice karo">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Job Role</label>
            <input className="input-field" type="text" name="role" value={form.role}
              onChange={handleChange} placeholder="Frontend Developer, Marketing Manager..." required />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <textarea className="input-field" name="question" value={form.question} onChange={handleChange}
              placeholder="Tell me about yourself..." rows="4" required />
          </div>
          <div className="form-group">
            <label>Aapka Answer</label>
            <textarea className="input-field" name="answer" value={form.answer} onChange={handleChange}
              placeholder="Apna jawab yahan likho..." rows="4" required />
          </div>
