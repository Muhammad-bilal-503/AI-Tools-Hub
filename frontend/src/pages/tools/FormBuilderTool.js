...
export default function FormBuilderTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/form-builder', form);
  ...
  return (
    <ToolPage icon="📋" title="AI Accessible Form Builder" desc="Form describe karo — WCAG-compliant HTML pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Form Describe Karo (English mein)</label>
            <textarea className="input-field" name="formDescription" value={form.formDescription} onChange={handleChange}
              placeholder="I need a contact form with name, email, phone, message, and a submit button with validation..." rows="4" required />
          </div>
