...
export default function FakeNewsTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/fake-news', form);
  ...
  return (
    <ToolPage icon="📰" title="AI Fake News Detector" desc="Article paste karo — credibility score pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Article Text Paste Karo</label>
            <textarea className="input-field" name="articleText" value={form.articleText} onChange={handleChange}
              placeholder="News article yahan paste karo..." rows="4" required />
          </div>
