...
export default function NewsTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/news-digest', form);
  ...
  return (
    <ToolPage icon="📡" title="AI News Digest" desc="Articles paste karo — summarized digest pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Aapki Interests</label>
            <input className="input-field" type="text" name="topics" value={form.topics}
              onChange={handleChange} placeholder="Technology, Sports, Politics, Business..." required />
          </div>
          <div className="form-group">
            <label>News Articles Paste Karo</label>
            <textarea className="input-field" name="articles" value={form.articles} onChange={handleChange}
              placeholder="Article 1 headline + text...
Article 2..." rows="4" required />
          </div>
