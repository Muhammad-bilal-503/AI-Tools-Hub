...
export default function ContentStudioTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/content-studio', form);
  ...
  return (
    <ToolPage icon="✍️" title="AI Content Marketing Studio" desc="Blog, Instagram, Twitter, LinkedIn — sab ek saath">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Topic / Product</label>
            <input className="input-field" type="text" name="topic" value={form.topic}
              onChange={handleChange} placeholder="Mera naya coffee shop, AI course..." required />
          </div>
          <div className="form-group">
            <label>Target Audience</label>
            <input className="input-field" type="text" name="audience" value={form.audience}
              onChange={handleChange} placeholder="Young professionals, students, entrepreneurs..." required />
          </div>
          <div className="form-group">
            <label>Tone</label>
