...
export default function LegalSummaryTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/legal-summary', form);
  ...
  return (
    <ToolPage icon="⚖️" title="AI Legal Document Summarizer" desc="Contract paste karo — simple English mein samjho">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Document Text Paste Karo</label>
            <textarea className="input-field" name="documentText" value={form.documentText} onChange={handleChange}
              placeholder="Contract, NDA, Terms of Service..." rows="4" required />
          </div>
