...
export default function SentimentTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/sentiment', form);
  ...
  return (
    <ToolPage icon="📊" title="AI Sentiment Analyzer" desc="Product reviews paste karo — feature-level analysis pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Product Reviews Paste Karo</label>
            <textarea className="input-field" name="reviews" value={form.reviews} onChange={handleChange}
              placeholder="Review 1: Great battery life...
Review 2: Design is good but..." rows="4" required />
          </div>
