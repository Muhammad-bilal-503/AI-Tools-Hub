...
export default function LearningPathTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/learning-path', form);
  ...
  return (
    <ToolPage icon="🎓" title="AI Learning Path Generator" desc="Personalized syllabus banao kisi bhi topic ke liye">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Kya sikhna chahte hain?</label>
            <input className="input-field" type="text" name="topic" value={form.topic}
              onChange={handleChange} placeholder="React.js, Machine Learning, Photography..." required />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            <label>Aapka Goal</label>
            <textarea className="input-field" name="goal" value={form.goal} onChange={handleChange}
              placeholder="Job lena chahta hoon, freelancing, personal project..." rows="4" required />
          </div>
