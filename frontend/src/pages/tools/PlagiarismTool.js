...
export default function PlagiarismTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/plagiarism', form);
  ...
  return (
    <ToolPage icon="🔎" title="AI Plagiarism Checker" desc="Text paste karo — AI-written content detect karo">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Text Paste Karo</label>
            <textarea className="input-field" name="text" value={form.text} onChange={handleChange}
              placeholder="Essay ya content yahan paste karo..." rows="4" required />
          </div>
