...
export default function ScraperTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/scraper', form);
  ...
  return (
    <ToolPage icon="🕷️" title="AI Web Scraper Generator" desc="Plain English mein batao — scraper code pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Website URL</label>
            <input className="input-field" type="text" name="url" value={form.url}
              onChange={handleChange} placeholder="https://example.com/products" required />
          </div>
          <div className="form-group">
            <label>Kaunsa Data Chahiye?</label>
            <textarea className="input-field" name="dataDescription" value={form.dataDescription} onChange={handleChange}
              placeholder="Product names, prices, aur ratings chahiye..." rows="4" required />
          </div>
