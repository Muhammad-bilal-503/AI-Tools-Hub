...
export default function AccessibilityTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/accessibility', form);
  ...
  return (
    <ToolPage icon="♿" title="AI Accessibility Auditor" desc="HTML paste karo — WCAG issues aur fixes pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>HTML Code Paste Karo</label>
            <textarea className="input-field" name="htmlCode" value={form.htmlCode} onChange={handleChange}
              placeholder="<div>...your HTML...</div>" rows="4" required />
          </div>
