...
export default function MeetingTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/meeting-summary', form);
  ...
  return (
    <ToolPage icon="📋" title="AI Meeting Summarizer" desc="Meeting transcript paste karo — structured notes pao">
      <div className="card" style={{ marginBottom: '20px' }}>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>Meeting Transcript Paste Karo</label>
            <textarea className="input-field" name="transcript" value={form.transcript} onChange={handleChange}
              placeholder="John: We need to launch by Friday...
Sarah: I can handle the design..." rows="4" required />
          </div>
